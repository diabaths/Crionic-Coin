...
#include <wallet/wallet.h>  // Crionic: Hive
#include <rpc/server.h>     // Crionic: Hive
#include <base58.h>         // Crionic: Hive
#include <sync.h>           // Crionic: Hive

static CCriticalSection cs_solution_vars;
std::atomic<bool> solutionFound;            // Crionic: Hive: Mining optimisations: Thread-safe atomic flag to signal solution found (saves a slow mutex)
std::atomic<bool> earlyAbort;               // Crionic: Hive: Mining optimisations: Thread-safe atomic flag to signal early abort needed
CBeeRange solvingRange;                     // Crionic: Hive: Mining optimisations: The solving range (protected by mutex)
uint32_t solvingBee;                        // Crionic: Hive: Mining optimisations: The solving bee (protected by mutex)
...
void BlockAssembler::resetBlock()
{
    ...
    fIncludeBCTs = true;    // Crionic: Hive
...
// Crionic: Hive: If hiveProofScript is passed, create a Hive block instead of a PoW block
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fMineWitnessTx, const CScript* hiveProofScript)
{
    ...
    // Crionic: Hive: Make sure Hive is enabled if a Hive block is requested
    ...
    // Crionic: Don't include BCTs in hivemined blocks
    if (hiveProofScript)
        fIncludeBCTs = false;
    ...
    // Crionic: Hive: Create appropriate coinbase tx for pow or Hive block
    if (hiveProofScript) {
    ...
    // Crionic: Hive: Choose correct nBits depending on whether a Hive block is requested
    if (hiveProofScript)
    ...
    // Crionic: Hive: Set nonce marker for hivemined blocks
...
{
    const Consensus::Params& consensusParams = Params().GetConsensus(); // Crionic: Hive
    ...
    for (const CTxMemPool::txiter it : package) {
        ...
        if (!fIncludeWitness && it->GetTx().HasWitness())
            return false;
        // Crionic: Inhibit BCTs if required
...
// Crionic: Hive: Bee management thread
void BeeKeeper(const CChainParams& chainparams) {
    ...
    try {
        while (true) {
             // Crionic: Hive: Mining optimisations: Parameterised sleep time
...
// Crionic: Hive: Mining optimisations: Thread to signal abort on new block
...
// Crionic: Hive: Mining optimisations: Thread to check a single bin
...
// Crionic: Hive: Attempt to mint the next block
    ...
    {   
        ...
        *phash = block.GetHashYespower();
        ...
        if ((nNonce & 0xfff) == 0 || miner->fKill > 0) {
            if (miner->fKill > 0)
                LogPrintf("Crionic miner kill flag > 0\n");
            return false;
        }
    }
...
static bool ProcessBlockFound(const CBlock* pblock, const CChainParams& chainparams)
{
    ...
    // Found a solution
    {
        LOCK(cs_main);
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash()) {
            LogPrintf ("Crionic Miner: generated block is stale\n");
            return false;
        }
    }
    ...
    if (!ProcessNewBlock (chainparams, ptr, true, nullptr)) {
        LogPrintf ("Crionic Miner: ProcessNewBlock, block not accepted\n");
        result =  false;
    }
...
void static CRNCMiner(MinerInfo* miner, const CChainParams& chainparams)
{
    LogPrintf("Crionic Miner started\n");
    SetThreadPriority (THREAD_PRIORITY_LOWEST);
    RenameThread ("crnc-miner");
...
//    }
    ...
    try {
      ...
      //      }
		...
		pblocktemplate.reset (CreateNewBlock (chainparams, coinbaseScript->reserveScript));
            ...
            {
                LogPrintf("Crionic Miner runtime error: %s\n", e.what());
                LogPrintf("Crionic Miner: Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                MilliSleep(4*1000);
                vpwallets[0]->GetScriptForMining (coinbaseScript);
                continue;
            }
            ...
            LogPrintf("Running Crionic Miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
            ...
            while (true) {
                ...
                if (ScanHash (miner, pblock, nNonce, &hash))
                {
                    if (UintToArith256(hash) <= hashTarget)
                    {
                        ...
                        LogPrintf("Crionic Miner: proof-of-work found  \n  hash: %s  \ntarget: %s\n",
    ...
    {
        LogPrintf("Crionic Miner terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
...
void GenerateCRNC(bool fGenerate, int nThreads, const CChainParams& chainparams)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CRNCMiner, vMiners[i], boost::cref(chainparams)));
