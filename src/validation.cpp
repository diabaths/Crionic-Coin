...
#include <miner.h>  // Crionic: Hive
#include <merkleblock.h> // Crionic: Hive for merkle transaction check in block

#if defined(NDEBUG)
# error "Crionic cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Lightning Signed Message:\n";   // Crionic: Should still use LTC's strMessageMagic so that pre-fork sigs validate
                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache)
{
...

    {
        ...
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s CRNC additional fees, %d delta bytes\n",
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
   ...
   /* if (nHeight == consensusParams.lastScryptBlock + 1)
    ...
    // Crionic: Slow-start the first n blocks  blocks to prevent early miners having an unfair advantage
...
// Crionic: Hive: Return the current cost for a single worker bee
...
void static UpdateTip(const CBlockIndex *pindexNew, const CChainParams& chainParams) {
    ...
    if (!IsInitialBlockDownload())
    {
        ...
        for (int i = 0; i < 100 && pindex != nullptr; i++)
        {
            ...
            // Crionic: Hive: Don't warn about unexpected version in Hivemined blocks
...
static bool CheckBlockHeader(const CBlockHeader& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW = true)
{
    ...
    if (nHeight >= nSpeedFork) {
	    ...
	    // Crionic: Hive: Check PoW or Hive work depending on blocktype
...
bool CheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // Check transactions
    for (const auto& tx : block.vtx)
        if (!CheckTransaction(*tx, state, true))    // Crionic: Fix CVE-2018-17144
...
// Crionic: Hive: Check if Hive is activated at given point
...
// Crionic: Hive: Check if Hive 1.1 is activated at given point
...
// Crionic: Hive: Check if Hive 1.2 is activated at given point
...
// Crionic: Hive: Get the well-rooted deterministic random string (see whitepaper section 4.1)
...
// Crionic: Hive: Get tx by given hash, from a block at given chain height
...
//	return true;
    ...
    // Crionic: Hive: Check appropriate Hive or PoW target
